...
import { toggleBrand, toggleClear, toggleStock } from "../../redux/actions/filterAction";
import fetchProducts from "../../redux/thunk/Products/fetchProducts";

const Home = () => {
  const dispatch = useDispatch()
  const { brands, stock, clear } = useSelector(state => state.filter.filters)
  ...
  return (
    <div className='max-w-7xl gap-14 mx-auto my-10'>
      <div className='mb-10 flex justify-end gap-5'>
        ...
        <button
          className={`border px-3 py-2 rounded-full font-semibold ${clear ? activeClass : null}`}
          onClick={() => dispatch(toggleClear())}
        >
          Clear filter
        </button>
