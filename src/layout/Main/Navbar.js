...
import { searchProduct, searchText } from "../../redux/actions/filterAction";

const Navbar = () => {
  const dispatch = useDispatch()
  const searchKeyword = useSelector(state => state.filter.keyword)
      ...
      <ul className='h-full  mx-auto flex justify-between items-center gap-3 font-semibold text-indigo-900'>
        ...
        <li className='flex bg-white mx-auto h-8 w-full max-w-lg  rounded-full pr-3'>
          <input
            ...
            name='search'
            id='search'
            onBlur={(e) => dispatch(searchText(e.target.value))}
          />
          <button onClick={() => dispatch(searchProduct(searchKeyword))}>
            <BiSearchAlt />
          </button>
        </li>
        <li>
